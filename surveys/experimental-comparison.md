...
| Accuracy | 分类、抽取、公式计算 | ACE 在 FiNER / Formula 中使用的精确匹配或任务准确率 |
...
|---|---|
| 论文 | 论文标题和本仓库笔记链接 |
| 实验对象 | Search Agent / Tool-use Agent / Web Agent / Coding Agent / Memory Agent / Harness Agent / Context Engineering Agent 等 |
| 核心任务 | 问答、代码修复、网页操作、工具调用、客服环境、数据库操作、领域抽取、领域计算等 |
| 训练数据 | 人工数据、历史轨迹、synthetic tasks、成功轨迹、偏好数据、上下文 playbook 条目等 |
| 测试数据集 / 环境 | benchmark 名称、子任务名称、域内 / 域外划分 |
| 基础模型 | Llama、Qwen、GPT、Claude、DeepSeek 等，以及模型规模 |
| 训练方法 | SFT、RFT、DPO、PPO、GRPO、REINFORCE、无训练、context adaptation 等 |
| 对比方法 | baseline、ablation、previous methods |
| 评价指标 | EM、F1、Accuracy、Pass@k、success rate、reward、TGC / SGC、latency、token cost、人工评分等 |
| 主要结论 | 这篇实验最想证明什么 |

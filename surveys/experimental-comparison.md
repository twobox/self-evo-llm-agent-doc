...
这一类工作不直接提出一个新 agent，而是给已有 LLM Agent memory 机制建立分类框架。它的重点不是“某个 benchmark 上提升多少分”，而是回答 memory 机制如何从 raw trajectory storage，发展到 reflection，再发展到 cross-trajectory experience abstraction。

典型分析维度：

| 维度 | 内容 |
|---|---|
| 任务类型 | 综述 / 分类框架；覆盖长期任务、动态环境、持续学习场景中的 Agent Memory |
| 更新对象 | memory repository、trajectory、reflection result、explicit experience、implicit experience、hybrid experience |
| 核心阶段 | Storage：trajectory preservation；Reflection：trajectory refinement；Experience：trajectory abstraction |
| 关键机制 | active exploration、cross-trajectory abstraction、contrastive induction、action distillation、code encapsulation、gradient internalization |
| 训练方式 | 综述本身不训练模型；被综述方法可能包括无训练记忆检索、反思写入、skill library、fine-tuning、RL、hybrid internalization |
| 常见指标 | memory retrieval、memory utility、experience generalization、continual learning benefit、memory lifecycle robustness |
| 关键问题 | Agent memory 是否只是保存过去，还是能把多条轨迹抽象成可迁移经验？ |
...
| Memory mechanism benchmark collection | From Storage to Experience | Agent Memory / 综述整理 | 论文附录整理 memory 相关 benchmark；重点指出现有评测更偏 Storage / Reflection，对 Experience 抽象、泛化和生命周期评估不足 |
...
| Memory Taxonomy | Storage / Reflection / Experience | From Storage to Experience | 把 memory 机制从轨迹保存、轨迹反思推进到跨轨迹经验抽象；不是 baseline，而是分类坐标系 |
...
| Memory Retrieval | Agent Memory | 记忆系统是否检索到相关、不过期、不误导的 memory item |
| Memory Utility | Agent Memory | 检索到的记忆是否真的改善后续任务，而不是只提高相似度匹配 |
| Experience Generalization | Experience-based Agent | 从多条轨迹抽象出的经验能否迁移到新任务、新环境或新分布 |
| Memory Lifecycle Robustness | Long-term Agent Memory | 记忆写入、更新、删除、衰减、去重、合并是否能长期稳定工作 |
| Continual Learning Benefit | Experience / Self-Evolving Agent | 长期任务流中，经验是否带来持续收益，而不是污染或过拟合 |
...
From Storage to Experience 的重要提醒是：Agent Memory 的演化不是简单扩容，而是从 **Storage → Reflection → Experience** 逐步提高信息密度和迁移能力。

这带来几个新的实验设计问题：

- memory 是否只是保存 raw trajectory，还是经过 reflection 清洗？
- reflection 是单条轨迹总结，还是跨多条轨迹归纳？
- experience 是显式文本规则、skill / library，还是被训练进模型参数？
- agent 是否能主动探索，生成更有价值的经验来源？
- 现有 benchmark 是否真的评估了长期生命周期、动态环境和跨任务泛化？

后续读 memory / experience base / self-improving agent 论文时，可以增加一个检查项：**论文里的 memory 到底停留在“存储过去”，还是已经能形成“可迁移经验”？**
...
|---|---|
| 论文 | 论文标题和本仓库笔记链接 |
| 实验对象 | Search Agent / Tool-use Agent / Web Agent / Coding Agent / Memory Agent / Harness Agent / Context Engineering Agent / Self-Referential Agent / Memory Survey 等 |
| 核心任务 | 问答、代码修复、网页操作、工具调用、客服环境、数据库操作、领域抽取、领域计算、数学推理、程序级自修改、memory 机制分类、experience 抽象等 |
| 训练数据 | 人工数据、历史轨迹、synthetic tasks、成功轨迹、偏好数据、上下文 playbook 条目、validation feedback、memory repository、cross-trajectory experience 等 |
| 测试数据集 / 环境 | benchmark 名称、子任务名称、域内 / 域外划分、case study 环境、长期任务流、动态环境、memory benchmark |
| 基础模型 | Llama、Qwen、GPT、Claude、DeepSeek 等，以及模型规模 |
| 训练方法 | SFT、RFT、DPO、PPO、GRPO、REINFORCE、无训练、context adaptation、dynamic code modification、reflection、experience abstraction 等 |
| 对比方法 | baseline、ablation、previous methods、taxonomy stage |
| 评价指标 | EM、F1、Accuracy、Pass@k、success rate、reward、TGC / SGC、latency、token cost、人工评分、memory utility、experience generalization、鲁棒性统计等 |
| 主要结论 | 这篇实验最想证明什么；综述论文则记录它如何重组研究地图 |
| 复现性 | 是否开源代码、数据、模型、环境配置；综述论文是否维护官方资源列表 |

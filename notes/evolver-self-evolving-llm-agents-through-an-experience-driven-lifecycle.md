...
任务环境主要是 QA 检索式环境，交互是多步检索/推理式交互，不是开放动态环境或真实用户多轮交互。

核心也是，保存轨迹-》生成经验库-》使用经验（作为上下文）-》参数训练模型。	

核心动作2个：维护内部经验库、GRPO训练模型；前者提供知识，是上下文维度上的；后者是多步思考策略，是轨迹维度上的，类似工具调用的策略训练（训训练过程中，把知识具体内容mark掉了）。

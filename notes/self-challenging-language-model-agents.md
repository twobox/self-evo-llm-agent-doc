...
> 图源：论文 arXiv HTML Figure 1；论文许可证见 OpenReview 页面（CC BY 4.0）。以下图片仅用于读书笔记引用和学习说明。

![Figure 1: Overview of Self-Challenging Agent](https://arxiv.org/html/2506.01716v1/x1.png)

图 1 是理解 SCA 的总览图：同一个 Agent 先作为 **Task Challenger** 探索环境并生成任务与验证方法，再作为 **Task Executor** 解决这些自生成任务，并用 verification function 给出的 reward 训练自己。
...
![Figure 2: An example of a synthetic Code-as-Task](https://arxiv.org/html/2506.01716v1/x2.png)

图 2 展示了一个具体 CaT 任务长什么样：challenger 不是只生成 instruction，而是同时生成 verification function、example solution 和 failure cases。这样系统才能自动检查“正例能过、反例不能过”，把不可行或验证器过宽松的任务过滤掉。
...
![Figure 3: Ablation studies of different RL algorithms](https://arxiv.org/html/2506.01716v1/x3.png)

图 3 对应 RL 算法消融：offline 的 Rejection Fine-Tuning / DPO 已经能带来提升，online 的 PPO / GRPO 可以进一步提高，但稳定性和工程复杂度更高。这张图能帮助理解为什么论文主实验选择相对简单的 one-step REINFORCE / RFT 风格训练。
...
![Figure 4: Human annotations of synthetic task qualities](https://arxiv.org/html/2506.01716v1/x4.png)

图 4 是解释 CaT 为什么必要的关键证据：只生成 instruction 或只加 verification function 时，任务里仍有大量 false negative / false positive；加入 example solution 能减少不可行任务，但可能放大过宽松验证器；再加入 failure cases 后，错误奖励被进一步过滤。
...
> 图源：论文 arXiv HTML Figure 6；仅用于读书笔记引用和学习说明。

![Figure 6: Scaling analysis for SCA](https://arxiv.org/html/2506.01716v1/x6.png)

图 6 很适合支撑工程结论：在训练集上，少量任务配更多轨迹也能变好；但在 OOD 测试集上，只有任务数量和覆盖度足够大时，增加轨迹才会真正泛化。这说明自演化 Agent 不能只在少数题上反复 roll out，更要自动扩展任务分布。
...
- OpenReview / NeurIPS 页面：<https://openreview.net/forum?id=9yusqX9DpR>
- Hugging Face Papers：<https://huggingface.co/papers/2506.01716>
- 论文 Figure 1：<https://arxiv.org/html/2506.01716v1/x1.png>
- 论文 Figure 2：<https://arxiv.org/html/2506.01716v1/x2.png>
- 论文 Figure 3：<https://arxiv.org/html/2506.01716v1/x3.png>
- 论文 Figure 4：<https://arxiv.org/html/2506.01716v1/x4.png>
- 论文 Figure 6：<https://arxiv.org/html/2506.01716v1/x6.png>
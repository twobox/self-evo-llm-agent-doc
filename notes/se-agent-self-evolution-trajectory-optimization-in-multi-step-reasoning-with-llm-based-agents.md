...
进一步横向看，几篇文章虽然都在讲 self-evolving / self-improving agent，但“进化对象”并不一样：

| 工作 | 主要进化对象 | 知识/经验存储位置 | 后续复用方式 | 是否主要训练模型参数 |
|---|---|---|---|---|
| SE-Agent | 同一任务中的多条解题轨迹 | trajectory pool、`.traj` / `.tra`、`traj.pool`、轨迹摘要 | 下一轮 operator 读取历史轨迹，生成 system prompt / strategy guidance | 否 |
| EvolveR | 经验原则、任务轨迹、executor 能力 | experience base、训练数据、模型参数 | 检索经验 + 经验蒸馏 + SFT / RL 更新 executor | 是，包含参数更新 |
| ACE | 可演化上下文 / playbook / 策略经验 | 外部上下文经验库 | 下次任务检索并拼接到上下文 | 否，主要是上下文工程 |
| Self-Challenging | 自生成任务与可验证训练轨迹 | synthetic tasks、成功/失败轨迹、训练集 | 用于训练 executor，提高任务执行能力 | 是 |
...
> **不要误读 self-evolution：** 这里的“自演化”不是模型参数自己变强，也不是跨任务长期 memory 自动积累；它主要指在当前任务求解过程中，把多条轨迹作为可操作对象，经过反思、重组、评价和筛选，得到更好的下一代轨迹或最终 patch。
...
### 7.2.1 进化知识存在哪里，以后怎么用？

SE-Agent 的“进化知识”主要不是存进模型参数，而是存进 **trajectory pool / 轨迹池**。更工程化地说，它保存的是每次尝试的执行过程和摘要，而不是抽象成一个长期通用经验库。

结合论文和开源实现，可以把存储分成几类：

| 存储对象 | 典型文件 / 数据结构 | 记录什么 | 用途 |
|---|---|---|---|
| 原始轨迹 | `.traj` | agent 与环境交互的完整过程 | 事后复盘、压缩、分析 |
| 压缩轨迹 | `.tra` | 去冗余后的关键轨迹信息 | 降低存储和 LLM 分析成本 |
| 问题描述 | `.problem` | 当前 issue / task 的描述 | 让后续 operator 知道任务上下文 |
| 预测结果 | `.pred` / patch | 本轮生成的修复结果 | 判断是否成功，辅助评价轨迹 |
| 轨迹池 | `traj.pool` | 每个实例、每轮 iteration 的轨迹摘要 | 下一轮生成策略提示、风险提示或重组材料 |
...
→ 写入 traj.pool
→ 下一轮 operator 读取历史轨迹
→ 生成新的 system prompt / strategy guidance
→ 当前 agent 用增强提示继续解题
```

因此，它的复用方式不是“训练后模型记住了”，而是：**下一轮执行前，外层 operator 读取历史轨迹，把失败经验、有效片段、风险点转成提示词，再注入当前轮 agent 的 system prompt。**

这和 EvolveR、ACE 的区别很关键：

- EvolveR 更像“维护经验库 + 蒸馏/训练 executor”；
- ACE 更像“维护可检索、可演化的上下文经验”；
- SE-Agent 更像“当前任务内维护轨迹池，并在下一轮测试时搜索中复用”。

### 7.2.2 论文机制与代码实现的对应关系

为了避免只停留在概念层，可以把论文里的几个机制和开源代码中的工程对象对应起来看：

| 论文中的概念 | 开源实现中的对应模块 / 文件 | 我的理解 |
|---|---|---|
| trajectory pool | `SE/trajectories/...`、`traj.pool`、`TrajPoolManager` | 保存多轮、多实例的轨迹和轨迹摘要 |
| trajectory compression | `.traj` → `.tra` | 把完整交互轨迹压缩成更适合后续分析的格式 |
| operator preprocessing | `SE/operators/*` | 每轮执行前读取历史结果，生成增强参数 |
| alternative strategy | `alternative_strategy.py` | 基于最近失败尝试生成“不同的”解题策略 |
| risk-aware guidance | `traj_pool_summary.py` | 总结多轮失败轨迹中的盲区和风险点 |
| system prompt injection | `iteration_x/system_prompt/*.yaml` | 把演化得到的策略写入下一轮 agent 的系统提示 |
| next iteration execution | `basic_run.py` / `se_run.py` | 使用 operator 生成的新配置继续求解 |

这个对应关系说明：SE-Agent 的“知识复用”不是神秘机制，本质上是一个外层控制系统：**保存轨迹 → 分析轨迹 → 生成提示 → 再执行 agent**。
...
用更抽象的伪代码表示，SE-Agent 可以概括为：

```text
Input: task x, base agent A, evolution rounds R

1. Generate initial trajectory pool P = {τ1, τ2, ..., τn}
2. For r = 1 ... R:
   a. Revision:
      revise weak / failed trajectories in P
   b. Recombination:
      combine useful segments, strategies, or evidence across trajectories
   c. Refinement:
      evaluate trajectories with task completion, reasoning quality, and efficiency
      keep high-quality and diverse trajectories
   d. Update trajectory pool P
3. Return the highest-reward trajectory / patch
```

这个流程的重点不是“多跑几次再投票”，而是：**每一代轨迹会被分析、改写、组合和筛选，下一代轨迹不是独立采样出来的，而是继承了上一代轨迹池中的信息。**
...
从工程上看，SE-Agent 的 trajectory pool 更像“当前任务求解过程中的工作记忆 / 搜索池”，而不是跨任务长期记忆库。它会保存这个任务的多轮尝试，并帮助下一轮避开同样的失败路径；但如果不额外设计经验抽取、去重、检索、压缩和跨任务索引机制，它不会自然变成可长期复用的经验库。

这也解释了为什么 SE-Agent 和 ACE / EvolveR 读起来相似，但技术侧重点不同：

```text
SE-Agent: 轨迹池内演化，重点是当前任务的测试时优化
ACE: 上下文/经验库演化，重点是下次任务如何用经验
EvolveR: 经验库 + 训练闭环，重点是经验如何沉淀进 executor
```
...
7. **长期复用能力不足**：如果只使用当前论文的轨迹池机制，它更偏 episode-level / task-level 的演化；要变成真正跨任务自进化系统，还需要额外的经验抽取、检索、版本管理和训练/蒸馏机制。
...
如果后续把这种轨迹演化结果再沉淀成经验库、训练数据或可复用 skill，就可能和 EvolveR、Self-Challenging、Harness Updating、ACE 等方向进一步连接起来。
